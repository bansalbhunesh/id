...
| `GET /model/sme-benchmark` | Real **SBA small-business** charge-off outcomes: holdout AUC/KS **plus out-of-distribution generalisation** on a differently-distributed real sample, monotone constraints, native exact TreeSHAP, and explicit honesty caveats. |
...
- Real small-business outcome benchmark: `GET /model/sme-benchmark` trains the same monotone-constrained, calibrated, native-exact-TreeSHAP methodology on **real SBA 7(a) small-business charge-offs** (holdout ROC-AUC **0.91**, KS **0.71**) and validates it **out of distribution** on a 1,159-loan differently-distributed real sample (ROC-AUC **0.94**, PSI **0.63**). Reproduce with `python backend/model_training/train_sme_pd_model.py`. This is the closest public real-outcome proxy to MSME credit risk and the only evidence in the field validated on real small-business defaults out of distribution; it is a curated, class-balanced research sample and a loan-structure benchmark, complementary to the alternate-data conduct pillars, and explicitly not an IDBI production calibration.

...
- Includes a model card, deterministic fallback logic, 50 passing tests, a calibrated monotonic XGBoost PD champion with a logistic challenger, untouched holdout evidence, protected decisioning routes, pseudonymous hash-chained audit access, GitHub Actions, Render deployment, and `render.yaml` Blueprint.

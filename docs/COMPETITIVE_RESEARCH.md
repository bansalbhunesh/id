...
- Includes a model card, deterministic fallback logic, 61 passing tests, a calibrated monotonic XGBoost PD champion with a logistic challenger, untouched holdout evidence, a dated-outcome temporal validation contract, fail-closed pilot promotion gates, protected decisioning routes, pseudonymous hash-chained audit access, GitHub Actions, Render deployment, and `render.yaml` Blueprint.

...
| AI/ML or data-driven implementation | Public requirements emphasize AI/ML/data-driven banking solutions and implementation readiness. | The project includes a transparent five-pillar score plus a real outcome-trained logistic PD model (real default-outcome label, reproducible held-out OOT AUC 0.745) with exact Shapley attribution, optional XGBoost/LightGBM + SHAP runtime path, validation metrics, and model status endpoint. |
| Sandbox APIs and datasets | Public summaries say shortlisted teams get sandbox APIs, synthetic datasets, cloud resources, and mentorship after shortlisting. | The repo does not pretend to have private IDBI sandbox credentials. It already exposes `/sandbox/score` and `/sandbox/recalibration/report` so official AA/GST/UPI/EPFO feeds can be plugged in when access is granted. |
| Technical implementation readiness | Evaluation summaries mention feasibility, scalability, business impact, and technical implementation. | Single FastAPI service, static frontend, Render Blueprint, Dockerfile, GitHub Actions, 40 tests, live Render deployment, regenerated screenshots, and submission deck PDF are included. |
| Governance and compliance seriousness | Banking solutions are expected to be deployable, scalable, and credible in a regulated context. | Governance is visible in-product: bearer-token/RBAC-gated audit access, hash-chained tamper-evident audit log, enforced consent, audit reconstruction, reason codes, policy guardrails, OOT validation, PSI drift, reason-code stability, fairness gap checks, model card, and deterministic memo fallback. See `docs/SECURITY_COMPLIANCE.md`. |

...
| Technical implementation readiness | Evaluation summaries mention feasibility, scalability, business impact, and technical implementation. | Single FastAPI service, static frontend, Render Blueprint, Dockerfile, GitHub Actions, 50 tests, live Render deployment, model artifacts/evaluation, screenshots, and submission deck PDF are included. |
| Governance and compliance seriousness | Banking solutions are expected to be deployable, scalable, and credible in a regulated context. | Governance is visible in-product: underwriter/auditor roles, source-scoped consent, pseudonymised restart-safe audit chain, score/PD/policy separation, random-holdout metrics with intervals, explicit OOT gap, PSI, fairness gaps, model card, and deterministic memo fallback. |
